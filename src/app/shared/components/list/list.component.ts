...
import { Column, ButtonGroup } from '../../classes/models';
...
export class ListComponent implements OnInit, OnChanges {
  ...
  @Input() sortable = false; // allow sort
  @Input() filterable = false; // allow filtering
  ...
  filterChanged(filterValue: string) {
    const res = this.getFilteredArray(
      this.data,
      filterValue,
      this.columns,
      this.filters
  ...
  keyIsFilterableColumn(columns: any[], k: string): boolean {
    ...
    if (columns.some((c) => c.name === k && !c.hidden)) {
      return true;
    }
